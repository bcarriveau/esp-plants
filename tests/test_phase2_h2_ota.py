...
def test_protocol_chunk_fits():
    s = read("shared/plantlink/plantlink_ota.h")
...
def test_h2_first_then_display_is_integrated_in_source():
    s = read("firmware/waveshare-hub/src/plants_ota_installer.cpp")
    assert '#include "h2_ota_client.h"' in s
    assert "espplants_h2_ota::updateForRelease" in s
    assert "return Result::FAILED" in s
    assert s.index("espplants_h2_ota::updateForRelease") < s.index(
        "esp_ota_get_next_update_partition"
    )
...
def test_manifest_keeps_schema_one_and_h2_metadata():
    ...
    assert '"h2":h2' in s
    assert "m5stack-unit-gateway-h2" in s