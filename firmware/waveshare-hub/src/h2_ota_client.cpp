...
constexpr char kWavePrefix[]="ESPPLANTS-WAVESHARE-";
constexpr char kH2Prefix[]="ESPPLANTS-H2-";
constexpr size_t kMaxH2Bytes=0xC0000u;
...
Result updateForRelease(const espplants_ota_installer::Release &release,espplants_ota_installer::ProgressCallback progress,char *message,size_t mc){if(strncmp(release.buildId,kWavePrefix,strlen(kWavePrefix))!=0){copyText(message,mc,"H2 target version could not be derived");return Result::FAILED;}const char*version=release.buildId+strlen(kWavePrefix);char expected[96]{},asset[128]{},shaAsset[140]{},url[512]{};snprintf(expected,sizeof(expected),"%s%s",kH2Prefix,version);snprintf(asset,sizeof(asset),"esp-plants-h2-%s.bin",version);snprintf(shaAsset,sizeof(shaAsset),"%s.sha256",asset);
